...
import { ArrowLeft, ArrowRight, Heart, Lock, RotateCcw, ShieldCheck, ShoppingCart, Truck } from "lucide-react";
            ...
            <div className="relative w-full h-80 sm:h-96 md:h-[620px] rounded-3xl overflow-hidden bg-black text-white shadow-xl">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent p-6 md:p-12 flex flex-col justify-end">
                ...
                <div className="mt-4 flex items-center gap-4">
                  <Link
                    href={`/products/${slugify(heroProducts[heroIndex]?.name || "")}`}
              ...
              {trendingKicks.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Heart className={`h-4 w-4 transition-all ${isLiked ? "fill-red-500 text-red-500" : "text-gray-900"}`} fill={isLiked ? "currentColor" : "none"} strokeWidth={2} />
                    </button>

                    <div>
                      <Link href={`/products/${slugify(product.name)}`} className="block">
      ...
      <section className="max-w-7xl mx-auto px-4 w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 my-6 sm:my-12">
          {[
            ...
            { icon: Lock, title: "Encrypted Checkout", desc: "Bank-level secure payments" },
          ].map((feature, idx) => (
            <div
              key={idx}
