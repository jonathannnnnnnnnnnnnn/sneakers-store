...
import { Heart, Lock, RotateCcw, ShieldCheck, ShoppingCart, Truck } from "lucide-react";
...
>
  <Heart className={`h-4 w-4 transition-all ${wishlistIds.map(String).includes(String(product.id)) ? "fill-red-500 text-red-500" : "text-gray-900"}`} fill={wishlistIds.map(String).includes(String(product.id)) ? "currentColor" : "none"} strokeWidth={2} />
</button>
