...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      // 3. Load all reviews for the global rating metric
      ...
      if (isMounted) {
        if (reviewError) {
          ...
          setReviews((reviewData || []) as AdminReview[]);
        }
      }

      // 4. Load All Auth Users
      await fetchUsers();
    };
...
const handleToggleRole = async (targetUser: UserProfile) => {
  ...
  return (
    <main className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 sm:py-8 lg:px-10">
      ...
      {/* Metric Cards */}
      <section className="mb-8 grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-5">
        {[
          {
          ...
          },
          {
            ...
            note: reviews.length ? `${reviews.length} review${reviews.length === 1 ? "" : "s"}` : "No reviews yet",
